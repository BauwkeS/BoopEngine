...
namespace boop
{
	...
	Texture::Texture(const std::string& text, TTF_Font* pFont, const glm::vec4& textColor)
		:m_Id{}
		, m_Width{ 10.0f }
		, m_Height{ 10.0f }
		, m_CreationOk{ false }
	...
	Texture::Texture(const std::string& text, const std::string& fontPath, int ptSize, const glm::vec4& textColor)
		:m_Id{}
		, m_Width{ 10.0f }
		, m_Height{ 10.0f }
		, m_CreationOk{ false }
	...
	void Texture::CreateFromString(const std::string& text, TTF_Font* pFont, const glm::vec4& color)
	{
		m_CreationOk = true;
		if (pFont == nullptr)
		{
	...
	//srcRect vec4 = x y z w == left bottom width height(w)
	void Texture::Draw(const glm::vec2& dstBottomLeft, const glm::vec4& srcRect) const
	{
		const float epsilon{ 0.001f };
		if (!m_CreationOk)
		{
			if (!(srcRect.z > epsilon && srcRect.w > epsilon)) // No srcRect specified
			{
				DrawFilledRect(glm::vec4{ dstBottomLeft.x, dstBottomLeft.y, m_Width, m_Height });
			}
			else
			{
				DrawFilledRect(glm::vec4{ dstBottomLeft.x, dstBottomLeft.y, srcRect.z, srcRect.w });
			}
		}
		else
		{
			glm::vec4 dstRect{ dstBottomLeft.x, dstBottomLeft.y, srcRect.z, srcRect.w };
			if (!(srcRect.z > epsilon && srcRect.w > epsilon)) // No srcRect specified
			{
				dstRect.z = m_Width;
				dstRect.w = m_Height;
	...
	void Texture::Draw(const glm::vec4& dstRect, const glm::vec4& srcRect) const
	{
		const float epsilon{ 0.001f };
		if (!m_CreationOk)
		{
		...
		else // srcRect specified
		{
			...
			textRight = (srcRect.x + srcRect.z) / m_Width;
			textTop = (srcRect.y - srcRect.w) / m_Height;
			textBottom = srcRect.y / m_Height;

			defaultDestHeight = srcRect.w;
			defaultDestWidth = srcRect.z;
		}
		...
		float vertexRight{};
		float vertexTop{};
		if (!(dstRect.z > 0.001f && dstRect.w > 0.001f)) // If no size specified use default size
		{
		...
		{
			vertexRight = vertexLeft + dstRect.z;
			vertexTop = vertexBottom + dstRect.w;
	...
	void Texture::DrawFilledRect(const glm::vec4& rect) const
	{
